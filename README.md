...
### 08.07 Tuesday
    ...
    - 앞으로 해야할 일
        ...
        * 강사 보러가기 기능 구현 / 8.12 완료
        * 반응형 디자인 구현 필요 / 8.12 완료
        * 회원가입 폼 수정 / 8.11 완료
        * 아이디 / 비밀번호 찾기 결과 페이지 구현 / 8.8 완료
        * 내비바 햄버거 버튼 만들기 / 8.11 완료
        * db 제약 조건 및 삭제 조건 작성 / 8.11 완료
...
### 개발하면서 느낀 점
    ...
    1. rails는 빠르게 개발이 가능한 프레임워크
    
    멋쟁이 사자처럼 6기 해커톤을 위해 약 3개월간 공부한 ruby on rails를 이용하여 개발을 했습니다.
    
    제가 이전에 Java 언어를 주로 공부했었고 웹에 관심이 있었기 때문에 Spring 프레임워크를 공부했었습니다.
    
    Spring이 확장성이 좋고 검색 자료가 많다는 점에서 좋은 프레임워크임은 분명하지만 환경설정 및 POJO를 구현하기 위한
    
    다양한 개념(DI, IoC, AoP 등)을 알아야 효율적으로 어플리케이션을 만들 수 있다는 점에서는 초보에게 어려울 수 있다고 생각합니다.
    
    이 점에서 rails는 gem을 이용한 각종 모듈을 손쉽게 자신의 어플리케이션에 이식 할 수 있다는 점,
    
    웹 개발을 위한 각종 api, 환경 설정 등 웹을 처음 접하는 입문자도 쉽게 어플리케이션을 만들 수 있는 점은 rails의 큰 메리트라고 생각합니다.
    
    2. front-end의 class
    
    주로 back-end를 공부하는 입장에서 이렇게 front-end를 다뤄본 적은 없었습니다.
    
    html / css / javascript(+jQuery)의 문법 등은 알고 있었지만 어떻게 이 언어들을 효율적으로 코딩할 수 있는지는 고민해보지 않았었습니다.
    
    때문에 처음 css를 작성했던 lecturer.scss 파일에 중구난방으로 class 이름들이 적혀있고(비슷한 이름도 존재...) 같은 스타일임에도 다른 클래스 이름으로 스타일링 되어있는 것도 있습니다.
    
    이런 이유로 css class 명명법인 BEM의 중요성을 느낄 수 있었습니다. (다만, 이번 프로젝트에서 제대로 활용하지 못한 부분이 있는 점은 아쉽다..)
    
    block, element, modifier에 대해 좀더 배울 수 있었던 시간이었습니다.